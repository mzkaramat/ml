...
public class RocInMemoryFitFn extends FitFn {
  private List<RankOnlineLearner> learners;
  private long seed;
  private int numIters;
  
  public RocInMemoryFitFn(List<RankOnlineLearner> learners, long seed, int numIters) {
  ...
  public void process(
    ...
    for (int i = 0; i < numIters; i++) {
      ...
      for (RankOnlineLearner learner : learners) {
        learner.update(positive, negative);
      }
    }
    
    for (int i = 0; i < learners.size(); i++) {
      RankOnlineLearner learner = learners.get(i);
